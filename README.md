# Pablo Hurtado — Portfolio

Portfolio profesional interactivo construido con React + Vite + Tailwind CSS v4.

## Desarrollo local

```bash
npm install
npm run dev
```

## Build de producción

```bash
npm run build
```

Genera el sitio estático en `dist/`.

## Deploy en Vercel

Este proyecto es un sitio estático de Vite (no Next.js), 100% compatible con el preset
"Vite" de Vercel sin configuración adicional:

1. Importa el repositorio en [vercel.com/new](https://vercel.com/new).
2. Vercel detecta automáticamente el framework Vite (build command `npm run build`,
   output directory `dist`).
3. Click en **Deploy**.

No se requiere `next.config.js` ni variables de entorno: es un sitio estático sin backend.
